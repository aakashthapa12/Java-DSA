// import java.util.*;
// public class main{
//     static int decreasing(int n){
//         System.out.println(n);

//         if(n == 1){
//             return 1;
//         }

//         return decreasing(n-1);
//     }
//     public static void main(String args[]){
//         int n = new Scanner(System.in).nextInt();

//         decreasing(n);
//     }
// }


// import java.util.*;
// public class main{
//     static int increasing(int n){
//         if(n == 1){
//             System.out.println(n);
//             return n;
//         }

//         increasing(n-1);
//         System.out.println(n);
//         return n;
//     }
//     public static void main(String args[]){
//         int n = new Scanner(System.in).nextInt();
        
//         increasing(n);
        
//     }
// }



// import java.util.*;
// public class main{
//     static void zigzag(int n){
//         if(n == 0){
//             return;
//         }
//         System.out.println("Pre"+n);
//         zigzag(n-1);
//         System.out.println("In"+n);
//         zigzag(n-1);
//         System.out.println("Post"+n);
//     }
//     public static void main(String args[]){
//         int n = new Scanner(System.in).nextInt();
//         zigzag(n);
//     }
// }



// import java.util.*;
// public class main{
//     static boolean iseven(int n){
//         if((n & 1) == 1) {
//             return false;
//         }
//         else{
//             return true;
//         }
//     }
//     public static void main(String args[]){
//         int n = new Scanner(System.in).nextInt();

//         System.out.println(iseven(n));
//     }
// }


// import java.util.*;
// public class main{

//     static int xpowern(int x, int n){
//         if(n == 0){
//             return 1;
//         }
//         int a = xpowern(x, n/2);

//         if((n & 1) != 1){
//             return a*a;
//         }
//         else{
            
//             return x * (a*a);
//         }

//     }
//     public static void main(String args[]){
//         int x = new Scanner(System.in).nextInt();
//         int n = new Scanner(System.in).nextInt();

//         System.out.println(xpowern(x,n));

//     }
// }


// public class main{
//     static void increasing(int arr[], int idx){
//         if(idx == arr.length){
//             return;
//         }
//         System.out.println(arr[idx]);
//         increasing(arr, idx+1);
//     }
//     public static void main(String args[]){
//         int arr[] = {10, 20, 30, 40, 50};
//         int idx = 0;
//         increasing(arr, idx);
//     }
// }




// public class main{
//     static void decreasing(int arr[], int idx){
//         if(idx == arr.length){
//             return;
//         }
//         decreasing(arr, idx+1);
//         System.out.println(arr[idx]);
//     }
//     public static void main(String args[]){
//         int arr[] = {10, 20, 30, 40, 50};
//         int idx = 0;
//         decreasing(arr, idx);
//     }
// }


// public class main{
//     static int max(int arr[], int idx){
//         if(idx == arr.length - 1){
//             return arr[idx];
//         }

//         int val = max(arr, idx+1);
//         if(val > arr[idx]){
//             return val;
//         }
//         else{
//             return arr[idx];
//         }
//     }
//     public static void main(String args[]){
//         int arr[] = { 22, 33, 4, 19, 7};
//         System.out.println(max(arr, 0));
//     }
// }



// public class main{
//     static int first_occurence(int arr[], int idx, int val){
//         if(idx == arr.length){
//             return -1;
//         }

//         if(arr[idx] == val){
//             return idx;
//         }
//         else{
//             int i = first_occurence(arr, idx+1, val);
//             return i;
//         }
//     }
//     public static void main(String args[]){
//         int arr[] = { 2, 3, 9, 8, 7, 6, 8, 5}; 
//         System.out.println(first_occurence(arr, 0, 8));
//     }
// }
  





// public class main{
//     static int last_occurence(int arr[], int idx, int val){
//         if(idx == arr.length){
//             return -1; 
//         }

//         int i = last_occurence(arr, idx+1, val);
//         if(i == -1){
//             if(arr[idx] == val){
//                 return idx;
//             }
//             else{
//                 return -1;
//             }
//         }
//         else{
//             return i;
//         }
//     }
//     public static void main(String args[]){
//         int arr[] = { 2, 3, 9, 8, 7, 6, 8, 5}; 
//         System.out.println(last_occurence(arr, 0, 8));
//     }
// }



// public class main{
//     static int[] allindices(int arr[], int val, int idx, int fsf){
//         if(idx == arr.length){
//             return new int[fsf];
//         }

//         int[]iarr;
//         if(arr[idx] == val){
//             iarr = allindices(arr, val, idx+1, fsf+1);
//             iarr[fsf] = idx;
//             return iarr;
//         }
//         else{
//             iarr = allindices(arr, val, idx+1, fsf);
//             return iarr;
//         }

//     }

//     static void print(int arr[]){
//         for(int i=0; i<arr.length; i++){
//             System.out.println(arr[i]);
//         }
//     }
//     public static void main(String args[]){
//         int arr[] = {2, 3, 9, 8, 7, 6, 8, 5};
//         int iarr[] = allindices(arr, 0, 8, 0);

//         print(iarr); 

//     }
// }


// public class main{
//     public static void main(String args[]){
//         String ans = "1234";
//         int a = Integer.parseInt(ans);
//         System.out.println(a);
        
//     }
// }







//{ Driver Code Starts
// Initial Template for Java

// import java.io.*;
// import java.util.*;
// import java.lang.*;

// class GFG{
//     public static void main(String args[])throws IOException
//     {
//         BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
//         int t = Integer.parseInt(in.readLine());
//         while(t-- > 0) {
//             int n = Integer.parseInt(in.readLine());
            
//             Solution ob = new Solution();
//             ArrayList<ArrayList<Integer>> ans = ob.nQueen(n);
//             if(ans.size() == 0)
//                 System.out.println("-1");
//             else {
//                 for(int i = 0;i < ans.size();i++){
//                     System.out.print("[");
//                     for(int j = 0;j < ans.get(i).size();j++)
//                         System.out.print(ans.get(i).get(j) + " ");
//                     System.out.print("] ");
//                 }
//                 System.out.println();
//             }
//         }
//     }
// }
// } Driver Code Ends


// User function Template for Java

// import java.util.*;
// public class main{
//     public static void main(String args[]){
//         int n = new Scanner(System.in).nextInt();
//         int chess[][] = new int[n][n];

//         ArrayList<ArrayList<Integer>> ans = new ArrayList<>();
//         helper(chess, 0, ans);
//         System.out.println(ans);
//     }

//     // static ArrayList<ArrayList<String>> nQueen(int n) {
//     //     // code here
        
//     //     int chess[][] = new int[n][n];
//     //     ArrayList<ArrayList<String>> ans = new ArrayList<>();
//     //     helper(chess, 0, ans);
//     //     return ans;
//     // }
    
//     static void saveBoard(int [][]chess, ArrayList<ArrayList<Integer>> ans){
//         String row = "";
//         ArrayList<Integer> newBoard = new ArrayList<>();
        
//         for(int i=0; i<chess.length; i++){
//             for(int j=0; j<chess.length; j++){
//                 if(chess[i][j] == 1){
//                     row = (j+1)+"";
//                 }
//             }


//             int a = Integer.parseInt(row);
//             newBoard.add(a);
//         }
        
//         ans.add(newBoard);
//     }
    
//     static void helper(int [][]chess, int row, ArrayList<ArrayList<Integer>> ans){
//         if(row == chess.length){
//             saveBoard(chess, ans);
//             return;
//         }
        
        
//         for(int col = 0; col<chess.length; col++){
//             if(isSafeforQueen(chess, row, col)){
//                 chess[row][col] = 1;
//                 helper(chess, row+1, ans);
//                 chess[row][col] = 0;
//         }
        
//     }
//     }
    
//      public static boolean isSafeforQueen(int [][]chess, int row, int col){
//         for(int i=row-1, j = col; i>=0; i--){
//             if(chess[i][j] == 1){
//                 return false;
//             }
//         }
//         for(int i=row-1, j = col-1; i>=0 && j>=0; i--, j--){
//             if(chess[i][j] == 1){
//                 return false;
//             }
//         }
//         for(int i=row-1, j = col+1; i>=0 && j<chess.length; i--, j++){
//             if(chess[i][j] == 1){
//                 return false;
//             }
//         }
//         return true;
//     }

// }




// class Solution{

//     static ArrayList<ArrayList<Integer>> nQueen(int n) {
//         // code here
        
//         int chess[][] = new int[n][n];
//         ArrayList<ArrayList<Integer>> ans = new ArrayList<>();
//         helper(chess, 0, ans);
//         return ans;
//     }
    
// // 1. This is our 1st function --> helper function: 

//     static void helper(int [][]chess, int row, ArrayList<ArrayList<Integer>> ans){
//         // This is our base case : when we have to return.
//         if(row == chess.length){ 
//             result(chess, ans); // This is our 4th function (See its code below)
//             return;
//         }
        
        
//         // * Important : These few lines of code holds the concept of Backtracking(very important). So, Understand this thoroughly.
//         for(int col = 0; col<chess.length; col++){
//             if(isSafeforQueen(chess, row, col)){
//                 chess[row][col] = 1; // while putting queen we will make chess[row][col] = 1.
//                 helper(chess, row+1, ans);
//                 chess[row][col] = 0; // while returning we have to make chess[row][col] = 0.
//         }
        
//     }

//     }

// // 2. This is our 2nd Function --> saveBoard Function : 
// //    This code is simple but sometime we forget how to convert String into Integer 
// //    as we need to return our ans in the form of ArrayList<ArrayList<Integer>> .

// // So, To convert String into Integer : we will use Integer.parseInt();

//     static void result(int [][]chess, ArrayList<ArrayList<Integer>> ans){
//         String row = "";
//         ArrayList<Integer> res = new ArrayList<>();
        
//         for(int i=0; i<chess.length; i++){
//             for(int j=0; j<chess.length; j++){
//                 if(chess[i][j] == 1){
//                     row = (j+1)+"";  // Here we store our values as string and then covert it into Integer.
//                 }
//             }

//             int a = Integer.parseInt(row);
//             res.add(a);  // here, Answer will be store in the form of ArrayList<Integer>
//         }
        
//         ans.add(res);  // here, Answer will be store in the form of ArrayList<ArrayList<Integer>>
//     }
    
    
// // 3. This is our 3rd function --> isSafeforQueen Function : 
// //    This will check if a queen is present in chess[i][j] (ith row,jth col) or not.
// //    Means, (i) - if chess[i][j] == 1 for ith row & jth col then, it wil return false.
// //    false --> indicates that queen is present in chess[i][j] and hence we cannot put queen on chess[i][j].

// //    Means, (ii) - if chess[i][j] != 1 for ith row & jth col then, it wil return true.
// //    false --> indicates that queen is not present in chess[i][j] and hence we can put queen on chess[i][j].

//      public static boolean isSafeforQueen(int [][]chess, int row, int col){
//         for(int i=row-1, j = col; i>=0; i--){
//             if(chess[i][j] == 1){
//                 return false;
//             }
//         }
//         for(int i=row-1, j = col-1; i>=0 && j>=0; i--, j--){
//             if(chess[i][j] == 1){
//                 return false;
//             }
//         }
//         for(int i=row-1, j = col+1; i>=0 && j<chess.length; i--, j++){
//             if(chess[i][j] == 1){
//                 return false;
//             }
//         }
//         return true;
//     }
// }

